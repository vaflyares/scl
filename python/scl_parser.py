...
class Parser:
    ...
    def parse_list_value(self) -> List[Any]:
        ...
        if element_type.type == TokenType.NUM:
            ...
            self.eat(TokenType.FL)
            parser_func = lambda: (self.eat(TokenType.FLOAT) if self.current_token().type == TokenType.FLOAT else self.eat(TokenType.NUMBER)).value
            ...
            if isinstance(value, bool):
                ...
                if not value:
                    ...
                    if isinstance(first_elem, bool):
                        type_name = "bool"
                        formatter = lambda x: 'true' if x else 'false'
                        ...
                        raise SCLParseError(f"Unsupported list element type: {type(first_elem)}")
                    items = ", ".join(formatter(item) for item in value)
